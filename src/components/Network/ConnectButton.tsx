...
const ConnectButton: React.FC<ConnectButtonProps> = ({ userId, onConnect }) => {
  ...
  const handleConnect = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(`${BASE_URL}/user/connections/request/${userId}`);
  ...
  return (
    <button
      className={`px-4 py-1.5 border rounded-full text-sm font-semibold transition-colors
      ${
        state === 'connect'
